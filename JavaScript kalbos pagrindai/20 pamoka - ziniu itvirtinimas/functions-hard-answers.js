...
console.group("25. Sukurkite funkciją, kuri pirmu parametru priimtų žodį, o antruoju - masyvą su raidėmis.");
{
  ...
  function filterLetters(str, lettersToRemove) {
    // Veiksmus atliekame su kiekviena raide, kurią reikia šalinti
    for (let i = 0; i < lettersToRemove.length; i++) {
      // Sukuriame kintamajį, kuriame išsaugome šalinamają raidę
      const letterToRemove = lettersToRemove[i];
      // kiuntamasis skirtas tikrinta reikšmių tapatumą, po bandymo šalinti raidę
      let temp = str;
      do {
        // Prieš šalinimą, sulyginame reikšmes, jog būtų galima atpažinti,
        // ar po bandymo pakeisti <letterToRemove> tuščia vieta, pasikeitė vertės:
        // <str> ir <temp>
        str = temp;
        // bandome pašalinti raidę <letterToRemove>
        temp = temp.replace(letterToRemove, '');
        // Tęsiame, tol kol <str> ir <temp> reikšmės po šalinimo nesutampa.
      } while (temp !== str);
    }
    return str;
  }
